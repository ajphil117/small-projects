...
namespace Assignment
{
    interface iToolLibrarySystem
    {
        ...
        void display(string aMemberContactNo); //given the contact phone number of a member, display all the tools that the member are currently renting

        void displayTools(int aToolType); //display all the tools of a tool type selected by a member

        void borrowTool(iMember aMember, iTool aTool); //a member borrows a tool from the tool library

        void returnTool(iMember aMember, iTool aTool); //a member returns a tool to the tool library
